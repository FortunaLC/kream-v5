<template>
  <header class="relative z-20 border-b bg-background/90 backdrop-blur">
    <UiContainer class="flex h-16 items-center justify-between lg:h-20">
          ...
          <UiNavigationMenuList class="gap-2">
            <UiNavigationMenuItem>
              <UiNavigationMenuLink as-child>
                <UiButton :to="$localePath('/')" variant="ghost" size="sm">
                  Home
                </UiButton>
              ...
              <UiNavigationMenuItem>
                ...
                <UiNavigationMenuContent>
                  <div class="lg:w-[750px] xl:w-[1000px]">
                    ...
                    <div class="col-span-full flex items-center justify-center bg-muted/20 py-5 dark:bg-muted/5">
                      <NuxtLink class="font-semibold text-primary hover:underline" to="#">
                        Looking for a new career? Get
                        in touch!
                      </NuxtLink>
                    </div>
                  </div>
      ...
      <div class="hidden items-center gap-3 lg:flex">
        ...
        <template v-if="!loggedIn">
          <UiButton :to="$localePath('/login')" variant="ghost" size="sm">
            Log in
          </UiButton>
          <UiButton :to="$localePath('/register')" size="sm">
            Sign up
          </UiButton>
        </template>
        <template v-else>
